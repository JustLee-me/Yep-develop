...
struct SearchedFeedCellLayout {
    ...
    init(feed: DiscoveredFeed) {
        ...
        if let skill = feed.skill {
            ...
            let rect = skill.localName.boundingRect(with: CGSize(width: 320, height: CGFloat(Float.greatestFiniteMagnitude)), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: YepConfig.FeedBasicCell.skillTextAttributes, context: nil)
        ...
        let _rect1 = feed.body.boundingRect(with: CGSize(width: SearchedFeedBasicCell.messageTextViewMaxWidth, height: CGFloat(Float.greatestFiniteMagnitude)), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: YepConfig.FeedBasicCell.textAttributes, context: nil)
