...
final class ConversationViewController: BaseViewController {
        ...
        case MessageMediaType.text.rawValue:
            ...
            if message.isIndicator {
                ...
                if let _rect = ChatTextCellLayout.textContentTextViewFrameOfMessage(message) {
                    ...
                    rect = message.textContent.boundingRect(with: CGSize(width: messageTextContentTextViewMaxWidth, height: CGFloat(Float.greatestFiniteMagnitude)), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: YepConfig.ChatCell.textAttributes, context: nil)
    ...
    @objc fileprivate func messagesMarkAsReadByRecipient(_ notification: Notification) {
        ...
        guard let lastRead = notification.object as? LastRead else {
            println("Error: messagesMarkAsReadByRecipient: \(String(describing: notification.object))")
            return
        }
